...
export function AcademiaWalletView() {
  ...
  // Atualiza os dados de PIX padrão caso venham salvos
  useState(() => {
    if (balanceData?.pix_settings?.pixKey && !pixKey) {
      setPixKey(balanceData.pix_settings.pixKey);
      if (balanceData.pix_settings.pixKeyType) setPixKeyType(balanceData.pix_settings.pixKeyType);
      if (balanceData.pix_settings.pixHolderName) setHolderName(balanceData.pix_settings.pixHolderName);
      if (balanceData.pix_settings.bankName) setBankName(balanceData.pix_settings.bankName);
    }
  });
  ...
  const filteredTransactions = transactions.filter((tx) => {
    const matchesSearch =
      ...
      (tx.pixKey && tx.pixKey.includes(searchTerm));

    if (!matchesSearch) return false;

    if (filterType === "ALL") return true;
    if (filterType === "DAY_PASS") return tx.type === "DAY_PASS" || tx.referenceType === "DAY_PASS";
    if (filterType === "ENROLLMENT") return tx.type === "ENROLLMENT" || tx.referenceType === "ENROLLMENT";
  ...
  const totalAccessCount = statementData?.summary.totalAccessesCount || 0;

  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Banner Principal da Carteira da Academia */}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-primary/10 border border-primary/30 text-primary flex items-center gap-1.5">
                <Building2 className="w-3.5 h-3.5" /> Painel Financeiro da Academia
              </span>
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 flex items-center gap-1">
                <ShieldCheck className="w-3 h-3" /> Catraca Finex Conectada
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold font-display text-foreground">
              Carteira & <span className="gradient-text">Recebimentos</span>
            </h1>
            <p className="text-muted-foreground text-sm sm:text-base mt-1 max-w-xl">
              Gerencie suas receitas por acessos avulsos (Day Pass), matrículas e solicite saques instantâneos via PIX.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-[11px] text-emerald-500 font-medium flex items-center gap-1 mt-1">
              <CheckCircle2 className="w-3 h-3 shrink-0" /> Liberado para transferência imediata
            </p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Acessos de catraca e matrículas
            </p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm relative overflow-hidden flex flex-col justify-between">
          ...
          <div>
            <p className="text-2xl sm:text-3xl font-display font-extrabold text-foreground">
              {totalAccessCount} acessos
            </p>
            <p className="text-[11px] text-muted-foreground mt-1">
              Liberados via Finex na recepção
            </p>
          </div>
        </div>
      </div>

      {/* Card Stripe Connect - Recebimento Automático & Split para Academia */}
      <StripeConnectCard role="ACADEMIA" />
      ...
      <div className="flex border-b border-border/60 gap-2 sm:gap-4 overflow-x-auto pb-px">
        ...
        >
          ...
          <span>Extrato de Recebimentos por Aluno</span>
      ...
      {/* ABA 1: EXTRATO DE RECEBIMENTOS DETALHADO */}
      {activeTab === "statement" && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <Input
                type="text"
                placeholder="Buscar por nome do aluno ou descrição..."
                value={searchTerm}
            ...
            <div className="flex gap-2 w-full sm:w-auto overflow-x-auto">
              {[
                { id: "ALL", label: "Todos" },
                { id: "DAY_PASS", label: "Day Passes (Catraca)" },
          ...
          {loadingStatement ? (
            ...
            <div className="text-center py-16 bg-card/40 border border-dashed border-border rounded-3xl p-8">
              ...
              <h3 className="font-bold text-lg text-foreground">Nenhuma entrada encontrada</h3>
              <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1">
                Quando os alunos realizarem acessos de Day Pass ou matrículas na sua academia, eles aparecerão detalhados aqui.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {filteredTransactions.map((tx) => {
                ...
                const isDayPass = tx.type === "DAY_PASS" || tx.referenceType === "DAY_PASS";
                const isEnrollment = tx.type === "ENROLLMENT" || tx.referenceType === "ENROLLMENT";
                ...
                return (
                  ...
                  >
                    {/* Aluno & Detalhes do Pagamento */}
                    <div className="flex items-center gap-3.5">
                      {/* Avatar do Aluno ou Ícone */}
                      {isWithdrawal ? (
                      ...
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="font-bold text-sm sm:text-base text-foreground">
                            {isWithdrawal
                              ...
                              : tx.sourceUserName || "Aluno Finex"}
                          </h4>

                          {/* Badges */}
                          {isDayPass && (
                            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 flex items-center gap-1">
                              <QrCode className="w-3 h-3" /> Day Pass (Catraca)
                            </span>
                          )}

                          {isEnrollment && (
                            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-cyan-500/10 border border-cyan-500/30 text-cyan-500 flex items-center gap-1">
                              <Building2 className="w-3 h-3" /> Matrícula Online
                            </span>
                          )}

                          {isWithdrawal && (
                            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-500/10 border border-amber-500/30 text-amber-500">
                        ...
                        <p className="text-xs text-muted-foreground flex flex-wrap items-center gap-2">
                          ...
                          {tx.paymentMethod && (
                            <>
                              <span>•</span>
                              <span className="uppercase text-[10px] font-semibold bg-muted px-1.5 py-0.5 rounded">
                                {tx.paymentMethod === "FINEX_WALLET" ? "Carteira Finex" : tx.paymentMethod}
                              </span>
                            </>
                          )}
                        </p>
                    ...
                    <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-2 sm:pt-0 border-border/40">
                      ...
                      <span className="text-[10px] font-medium text-muted-foreground">
                        {isWithdrawal
                          ? "Transferido via PIX"
                          : tx.fee && tx.fee > 0
                          ? `Bruto: R$ ${tx.amount.toFixed(2)} (Taxa: R$ ${tx.fee.toFixed(2)})`
                          : "Creditado no Saldo"}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border/60">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Receba os valores acumulados dos seus acessos diretamente via PIX sem tarifas.
                </p>
              </div>
            </div>

            <form onSubmit={handleWithdrawSubmit} className="space-y-6">
              {/* Saldo Disponível de Referência */}
              ...
              <div className="space-y-4 pt-2 border-t border-border/50">
                <h4 className="text-sm font-bold flex items-center gap-1.5 text-foreground">
                  <ShieldCheck className="w-4 h-4 text-primary" /> Dados da Chave PIX de Destino
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-muted-foreground">Nome do Titular da Conta</label>
                    <Input
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-muted-foreground">Instituição / Banco (Opcional)</label>
                    <Input
                      type="text"
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  />
                  <label htmlFor="saveDefaultPix" className="text-xs text-muted-foreground cursor-pointer">
                    Salvar esta chave PIX como padrão para transferências automáticas futuras
                  </label>
                </div>
              </div>
      ...
      {/* ABA 3: HISTÓRICO DE SAQUES REALIZADOS */}
      {activeTab === "history" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-lg text-foreground">Saques Realizados para Conta Bancária</h3>
          ...
          {!withdrawalsData || withdrawalsData.length === 0 ? (
            <div className="text-center py-16 bg-card/40 border border-dashed border-border rounded-3xl p-8">
              ...
              <p className="text-xs text-muted-foreground max-w-sm mx-auto mt-1">
                Quando você transferir saldo da sua carteira para sua conta via PIX, os comprovantes ficarão salvos aqui.
              </p>
              <Button
                type="button"
                variant="outline"
